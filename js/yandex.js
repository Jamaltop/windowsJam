...
function updateProgress() {
    let percent = Math.round((currentMusic.currentTime / currentMusic.duration) * 100) + '%';
...
Butload.addEventListener('click', () => {
    upMusic.style.display = 'flex';
});
...
file.addEventListener('change', (e) => {
    ...
    .map(a => `<button class="audio-item" data-src="${a.src}">${a.name}</button>`)
    .join('<br>');
